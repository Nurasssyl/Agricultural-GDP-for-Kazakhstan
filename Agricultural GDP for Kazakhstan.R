...
map <- ggplot() +
  ...
  coord_sf(crs = crs) +
  labs(
    title = "Сельскохозяйственный ВВП Казахстана",
    caption = ""
  ) +
  theme_for_the_win()
...
map2 <- ggplot() +
  ...
  coord_sf(crs = crs) +
  labs( # should be labs
    title = "Сельскохозяйственный ВВП на душу населения в Казахстане",
    caption = ""
  ) +
  theme_for_the_win()
...
ggsave("agricultural_gdp_map.png", plot = map, width = 10, height = 7, dpi = 300)
ggsave("agricultural_gdp_per_capita_map.png", plot = map2, width = 10, height = 7, dpi = 300)
